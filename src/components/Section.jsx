...
export function SectionHeading({ eyebrow, title, intro, className = '' }) {
  return (
    <Reveal className={`max-w-2xl ${className}`}>
      {eyebrow ? (
        <p className="flex items-center gap-3 text-[0.63rem] uppercase tracking-[0.22em] text-accent-deep">
          <span aria-hidden="true" className="h-px w-8 bg-accent" />
          {eyebrow}
        </p>
      ) : null}
      <h2 className="mt-4 font-serif text-[2rem] leading-[1.12] md:text-[2.6rem]">{title}</h2>
